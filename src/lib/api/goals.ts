  ...
  prompts: [
    {
      ...
      description: "Ask how to close the $6,800 gap over the next 6 months without cutting essentials.",
    },
    {
      id: "prompt-celebrate",
      label: "Accelerate debt freedom",
      description: "Explore how applying credit card rewards could retire the student loan two months early.",
    },
    {
      id: "prompt-family-plan",
      label: "Optimize vacation savings",
      description: "Get ideas to boost the family trip fund while keeping monthly transfers comfortable.",
    },
  ],
  insights: [
    {
  ...
  goals: [
    {
      ...
      aiSummary: "You are one month behind schedule; a $150 boost for the next four transfers closes the gap.",
    },
    {
      ...
      aiSummary: "Momentum looks great?stay the course and redirect reimbursements to finish early.",
    },
    {
...
export function buildGoalFromDraft(
  draft: GoalDraft,
  ...
  const currentAmountCents = clamp(Math.round(draft.startingAmountCents), 0, draft.targetAmountCents);
  ...
  if (milestones.length === 0) {
    milestones.push({
      ...
      achievedAt: currentAmountCents >= draft.targetAmountCents ? toISODate(referenceDate) : undefined,
    });
  }
  ...
  const nextContributionDate = toISODate(addDays(referenceDate, frequencyToDays(draft.plan.frequency)));
  const aiSummary = draft.motivation
    ? draft.motivation
    : `Stay consistent with contributions to reach ${draft.name}.`;
  const frequencyLabel = draft.plan.frequency === "monthly" ? "month" : draft.plan.frequency === "biweekly" ? "2 weeks" : "week";
...
export async function requestGoalRecommendation(
  ...
  if (remainingCents === 0) {
    ...
    if (monthlyGap > 0) {
      summaryParts.push(`You need roughly $${centsToDollars(monthlyGap)} more per month to stay on target.`);
    } else if (monthlyGap < 0) {
      summaryParts.push(`You are ahead by about $${centsToDollars(Math.abs(monthlyGap))} each month.`);
    } else {
  ...
  if (monthlyGap > 0) {
    actions.push({
      ...
      description: "Increase the next three contributions or redirect cash-back to close the shortfall.",
    });
  } else {
    actions.push({
  ...
  if (!goal.autopilot) {
    actions.push({
      ...
      description: "Switch to automated moves so every paycheck nudges the goal forward without manual effort.",
    });
  }
