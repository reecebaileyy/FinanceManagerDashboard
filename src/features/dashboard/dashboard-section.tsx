...
import type { DashboardMonthlySummary, DashboardOverviewPayload, GoalStatus } from "@lib/api/dashboard";
...
export function DashboardSection() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const overData = over.data?.current as { type?: string; column?: DashboardColumnKey } | undefined;
  ...
  const widgetContentMap = useMemo<Record<DashboardWidgetId, ReactNode> | null>(() => {
    ...
    const { monthlySummary, categorySpending, goals, alerts, quickActions, recentTransactions } = data;



    const goalProgressPercent = goals.length

      ? Math.round((goals.reduce((sum, goal) => sum + clampGoalProgress(goal.progress), 0) / goals.length) * 100)
    ...
    return {
          ...
          <CardBody className={styles.summaryBody}>
            ...
            <div className={styles.summaryTotals}>
              ...
              <div>
                ...
                <span className={`${styles.summaryDelta} ${varianceToneClass[monthlySummary.variance.tone]}`}>
            ...
            <div className={styles.metricList}>
              ...
              {categorySpending.length ? (
                ...
                <div className={patterns.emptyState}>Category insights will populate once transactions sync.</div>
              )}
            </div>
            ...
            {goals.length ? (
              ...
              <div className={styles.goalList}>
                ...
                {goals.map((goal) => {
                  ...
                  const statusToneClass = goalStatusToneClass[goal.status] ?? styles.goalStatusNeutral;
                  ...
                  return (
                    ...
                    <div key={goal.name} className={styles.goalItem}>
                      ...
                      <div className={patterns.progress}>
                        ...
                        <span className={patterns.progressBar} style={{ width: `${progressPercent}%` }} />
              ...
              <p className={styles.goalEmpty}>Set your first goal to start tracking your progress.</p>
            ...
            <div className={styles.transactionsList} role="table" aria-label="Recent transactions">
              ...
              {recentTransactions.length ? (
                recentTransactions.map((transaction) => (
                  <div key={`${transaction.merchant}-${transaction.date}`} className={styles.transactionsRow} role="row">
                ...
                <div className={patterns.emptyState}>Recent transactions will surface as activity is ingested.</div>
              )}
            </div>
      ...
      "alerts-reminders": (
        ...
        <Card>
          ...
          <CardHeader title="Alerts & reminders" subtitle={`You have ${alerts.length} new notifications`} />

          <CardBody className={styles.alertsBody}>

            {alerts.length ? (
              alerts.map((alert) => (
                <div key={alert.title} className={`${styles.alert} ${alertToneClassName[alert.tone]}`} >
                  <div className={styles.alertIcon} aria-hidden="true">{alert.icon}</div>
                  <div>
              ...
              <div className={patterns.emptyState}>No alerts yet. We will notify you as reminders come due.</div>
            )}
          </CardBody>
        ...
        <Card>
          ...
          <CardBody className={styles.quickActions}>
            ...
            {quickActions.length ? (
              ...
              <div className={patterns.emptyState}>Add quick actions to surface high-value workflows.</div>
            )}
          </CardBody>
    ...
    <section id="dashboard" className={styles.section}>
      ...
      <div className={styles.kpiGrid}>
        ...
        {kpis.length ? (
          ...
          <div className={patterns.emptyState}>KPI data will appear once platform analytics are flowing.</div>
        )}
      </div>
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={handleDragCancel}>
        ...
        <div className={styles.overviewGrid}>
          ...
          <DashboardColumn column="secondary" items={layout.secondary} isCustomizing={isCustomizing}>
