...
describe("TransactionsSection", () => {
  ...
  it("applies bulk tag updates to selected transactions", async () => {
    ...
    const table = screen.getByRole("table");
    const coffeeRow = within(table).getByText("Blue Bottle Coffee").closest("tr");
    const foodsRow = within(table).getByText("Whole Foods Market").closest("tr");

    expect(coffeeRow).not.toBeNull();
    expect(foodsRow).not.toBeNull();
    expect(within(coffeeRow as HTMLTableRowElement).getByText("family")).toBeInTheDocument();
    expect(within(foodsRow as HTMLTableRowElement).getByText("family")).toBeInTheDocument();
  });
  ...
  it("adds queued imports to the recent imports list", async () => {
    ...
    await waitFor(() => expect(screen.getByText("new-import.csv")).toBeInTheDocument());
  });
});
