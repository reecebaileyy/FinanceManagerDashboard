...
export function InsightsWorkspace() {
  ...
  >({
    ...
    onError: (error, variables, context) => {
      ...
      if (context) {
        setMessages((prev) =>
          prev.filter(
            (message) => message.id !== context.userMessage.id && message.id !== context.pendingMessage.id,
          ),
        );
    ...
    onSuccess: (result, _variables, context) => {
      ...
      setRecommendations((prev) => sortRecommendations(mergeRecommendations(prev, result.recommendations)));
  ...
  >({
    ...
    onMutate: (variables) => {
      ...
      setMessages((prev) =>
        prev.map((item) => (item.id === variables.messageId ? { ...item, feedback: variables.score } : item)),
      );
  ...
  const handleReset = () => {
    ...
    queryClient.invalidateQueries({ queryKey: insightsQueryKeys.overview() });
  };
                  ...
                  return (
                    <div key={message.id} className={rowClassName}>
                      <div className={bubbleClassName}>
                        {message.pending ? <span>Drafting your personalized insight…</span> : message.content}
                      </div>
          ...
          <Card>
            ...
            <CardBody className={styles.recommendationList}>
              {recommendations.length === 0 ? (
                <p className={styles.emptyState}>No recommendations yet. Check back after your next sync.</p>
              ) : (
                  ...
                  <article key={item.id} className={styles.recommendationItem}>
                    <header className={styles.recommendationHeader}>
                      ...
                      <div className={styles.recommendationTags}>
                        ...
                        {item.status !== "new" ? (
                          <span className={controls.pill}>{item.status === "acknowledged" ? "Acknowledged" : "Dismissed"}</span>
                        ) : null}
                      </div>
                    </header>
                    <p className={styles.recommendationBody}>{item.body}</p>
                    <footer className={styles.recommendationFooter}>
                      <Link href={item.actionUrl} className={classNames(controls.button, styles.linkButton)}>
                        Review details
                      </Link>
