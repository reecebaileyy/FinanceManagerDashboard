...
describe("GoalsSection", () => {
  it("renders overview metrics and active goals", async () => {
    ...
    expect(screen.getByText("Emergency Cushion")).toBeInTheDocument();
    expect(screen.getByText("Student Loan Snowball")).toBeInTheDocument();
  ...
  it("walks through the wizard and creates a new goal", async () => {
    ...
    const sourceInput = screen.getByLabelText("Source account") as HTMLInputElement;
    ...
    expect(screen.getByText("Dream Kitchen")).toBeInTheDocument();
