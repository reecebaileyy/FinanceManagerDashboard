...
export type TwoFactorCodePayload = { method: "code"; code: string };
export type TwoFactorBackupPayload = { method: "backup"; backupCode: string };
export type TwoFactorVerificationPayload = TwoFactorCodePayload | TwoFactorBackupPayload;

type TwoFactorErrors = Partial<{ code: string; backupCode: string }>;
...
export function TwoFactorSection({
  ...
  const destinationHint = useMemo(() => formatDestination(method, destination), [method, destination]);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setStatus("error");
      const text = error instanceof Error && error.message.trim().length > 0 ? error.message : "Unable to resend the code.";
      setMessage(text);
    } finally {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === "code") {
      ...
      try {
        ...
        setStatus("error");
        const text = error instanceof Error && error.message.trim().length > 0 ? error.message : "That code didn\'t work. Try again.";
        setMessage(text);
      } finally {
    ...
    try {
      ...
      setStatus("error");
      const text = error instanceof Error && error.message.trim().length > 0 ? error.message : "That backup code was invalid.";
      setMessage(text);
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <section id="two-factor" className={styles.section}>
      <div className={`${patterns.grid} ${patterns.gridCols2}`}>
        <Card>
          <CardHeader title="Two-factor authentication" subtitle={`Great to see you, ${greeting}`} />
          <CardBody>
            <form className={patterns.form} onSubmit={handleSubmit} noValidate>
              {message ? <p className={alertClassName}>{message}</p> : null}
              <p className={styles.helperText}>
                Enter the {mode === "code" ? "6-digit" : "one-time backup"} code from your {METHOD_LABEL[method] ?? "authenticator"}
                ...
                <div>
                  <label className={patterns.formLabel} htmlFor="two-factor-backup">
                    ...
                    <input
                      ...
                      required
                    />
                    {errors.backupCode ? <span className={styles.fieldError}>{errors.backupCode}</span> : null}
                  </label>
                  <div className={styles.backupActions}>
                    <button
                      type="button"
                    ...
                    {typeof backupCodesRemaining === "number" ? (
                      <span className={styles.backupHint}>{backupCodesRemaining} backup codes left</span>
                    ) : null}
                  </div>
                </div>
              )}
        ...
        <Card>
          ...
          <CardBody>
            <ul className={styles.infoList}>
              <li>Codes rotate every 30 seconds. If one fails, wait for the next before retrying.</li>
