...
function generateChallenge(email: string, displayName: string, redirectTo?: string, roleHint?: SignupRole): TwoFactorChallenge {
  return {
    id: typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `challenge-${Date.now()}`,
...
function resolveInitialSession(initialSession?: AuthSession | null, initialCookie?: string | null): AuthSession | null {
  if (initialSession) {
...
export function SessionProvider({ children, initialSession, initialCookie }: SessionProviderProps) {
  ...
  const completeTwoFactor = useCallback(
    async (submission: TwoFactorSubmission, options?: { redirectTo?: string }) => {
      ...
      const displayName = challenge.displayName || resolveDisplayName(challenge.email ?? DEFAULT_DISPLAY_NAME);
  ...
  const signIn = useCallback(
    async (credentials: SignInCredentials, options?: { redirectTo?: string }): Promise<SignInResult> => {
  ...
  const register = useCallback(
    async (payload: SignupPayload, options?: { redirectTo?: string }): Promise<SignInResult> => {
      ...
      if (requiresTwoFactor) {
        const challenge = generateChallenge(normalizedEmail, displayName, options?.redirectTo, payload.role);
        setState((previous) => ({ ...previous, pendingChallenge: challenge }));
        return { status: "needs_two_factor", challenge };
      }
      ...
      const nextSession = createSessionPayload({
        ...
        metadata: {
          isTwoFactorEnrolled: payload.role === "admin" ? true : shouldRequireTwoFactor(normalizedEmail, payload.role),
        },
      });
  ...
  }, [cancelTwoFactorChallenge, clearSession, completeTwoFactor, isAuthenticated, isDemo, register, signIn, signOut, startDemoSession, state.pendingChallenge, state.session, status]);
