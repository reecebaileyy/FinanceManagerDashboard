...
export function BudgetsSection() {
  ...
  const budgetMutation = useMutation<Budget, Error, SaveBudgetInput, BudgetMutationContext>({
    ...
    onError: (
      error: Error,
      _input: SaveBudgetInput | undefined,
  ...
  const handleExportBudgets = () => {
    try {
      const { fileName } = downloadBudgetsCsv(budgets, {
        referenceDate: referenceDate.toISOString(),
      });
      setExportStatus({
        state: 'success',
        message: `${budgets.length} budget${budgets.length === 1 ? '' : 's'} exported as ${fileName}.`,
      });
    } catch (unknownError) {
      console.error('Failed to export budgets', unknownError);
      setExportStatus({
        state: 'error',
        message: "We couldn't prepare the CSV. Try again in a moment.",
      });
    }
  };
            ...
            actions={
              <div className={styles.headerActions}>
                ...
                <button
                  ...
                  aria-label="Download budgets as CSV"
                >
                  Export CSV
                </button>
              </div>
          ...
          <CardBody className={styles.cardBody}>
            {exportStatus.state !== 'idle' && exportStatus.message ? (
              <div
                role="status"
                aria-live="polite"
                className={`${styles.exportBanner} ${
                  exportStatus.state === 'success'
                    ? styles.exportBannerSuccess
                    : styles.exportBannerError
                }`}
              >
              ...
              <div className={`${patterns.glass} ${styles.alertsPanel}`}>
                ...
                {alerts.length ? (
                  ...
                  <p className={styles.alertEmpty}>
                    You are pacing beneath threshold. We will surface new alerts here.
                  </p>
                )}
              </div>
